...
def main():
    ...
    port = os.environ.get("SERIAL_PORT", "COM31")
    ...
    # Now write 5 registers starting at 0x00
    length = 5
    inst.write_registers(0, [i for i in range(length)])  # start address, list of values
    # Now write to the max register location
    read_back = inst.read_registers(0, length)  # Read out 10 registers starting at address 0x00
