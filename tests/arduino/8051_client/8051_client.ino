...
void setup(){
    //USBSerial is already initialized by the Arduino framework, so we don't need to call USBSerial_begin()
...
void loop(){
    if(USBSerial_available()) {
        request = NULL;
        bmodbus_client_next_byte(&modbus1, micros(), USBSerial_read());
        ...
        if (request) { //Incoming modbus request
            if(dispatch(request->function, request->address, request->data, request->size) == 0){ //Only reply if we accepted the request
                while(USBSerial_available()){ //Clear the serial buffer if any bytes came in while processing (or were pending)
                    USBSerial_read();
                }
                modbus_uart_data_t * r = bmodbus_client_get_response(&modbus1); //Generate the payload from the data
                USBSerial_print_n(r->data, r->size); //Send the response back to the serial port
            }else{
